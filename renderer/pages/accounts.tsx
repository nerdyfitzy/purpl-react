...
import AccountWorkerModal from "../components/accounts/accountWorkerModal";
import Worker from "../components/accounts/worker";
...
function Home() {
  ...
  const selectAll = () => {
    addSelected([]);
    addSelected(gmails.map((prof) => prof.uuid));
  };
  ...
  const isBlurred = () => {
    if (showAccount) return { filter: "blur(3px)" };
    return {};
  };
  return (
    <React.Fragment>
        ...
        <div className='flex h-full' style={isBlurred()}>
          ...
          <div className='flex flex-col w-screen h-screen justify-start'>
            ...
            >
              ...
              <div className='font-semibold text-4xl'>Account Tool</div>
            </div>
