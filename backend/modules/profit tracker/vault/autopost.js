...
const token =
  "mfa.JpVhCl6xZ96d4gDxnPyV8Lgbf_hLETPFVSQ2J-0TVnLvTSYh6U3pcXRj9blkPsIxl7kd9daZbzZYodcweQ56";
...
const postListing = (message, market, bot, type) => {
  var options = {
    ...
    body: {
      content: message,
      nonce: randomstring.generate({ length: 18, charset: "1234567890" }),
      tts: false,
    },
    json: true,
  };
...
const monitor = (bot, lowprice, highprice, message) => {
  ...
  request(options, function (error, response, body) {
    ...
    if (message.author.id !== last) {
      ...
      dmUser(message.author.id, "hey! i wtb rush");
    }
  });
};
...
const dmUser = (id, message) => {
  ...
  request(options, function (error, response, body) {
    ...
    const options2 = {
      method: "POST",
      url: `https://discord.com/api/v8/channels/${
        JSON.parse(body).id
      }/messages`,
      headers: {
        authority: "discord.com",
